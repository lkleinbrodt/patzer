...
- **`eval/engine.py`:** Vectorized legal-move mask (`_apply_legal_move_mask`); incremental UCI token cache (`_MoveTokenCache`) for append-only play; regression tests `eval/test_patzer_engine.py` (unittest, stub model — no checkpoint required).
